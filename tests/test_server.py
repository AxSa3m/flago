...
from flago.models import ActionProposal, PendingActionStatus, WriteActionType
from flago.server import create_app
...
def test_admin_page_updates_all_config_form_and_lists_menus(tmp_path, monkeypatch) -> None:
    ...
    with TestClient(create_app(settings)) as client:
        ...
        monkeypatch.setattr(
            "flago.server.admin.service_status",
            lambda **kwargs: fake_service_status,
        )
        monkeypatch.setattr(
            "flago.server.admin.trigger_service_action",
        ...
        monkeypatch.setattr("flago.server.admin.build_model_router", lambda settings: FakeRouter())
        ...
        payload = {
            ...
            "env__FLAGO_BASE_URL": "https://flago.example.test",
            "env__FLAGO_AGENT_MODE": "agent",
            "env__FLAGO_WRITEBACK_ENABLED": "true",
            "env__FLAGO_WRITEBACK_CONFIRMATION_MODE": "low_risk_direct",
            "env__GEMINI_DISPLAY_NAME": "Flago专用 Gemini",
    ...
    assert "Flago只会连接你填入的这个应用" in setup_text
    ...
    assert "FLAGO_AGENT_MODE" not in setup_text
    assert "FEISHU_VERIFICATION_TOKEN" not in setup_text
    assert "flago.admin.open" in setup_text
    ...
    assert "FLAGO_BASE_URL" in admin_text
    ...
    assert "flago.writeback.undo" in admin_text
    assert "flago.admin.open" in admin_text
    ...
    assert "FLAGO_LOG_LEVEL" in admin_text
    ...
    assert "FLAGO_ADMIN_ENABLED" not in admin_text
    assert "cli_test" not in admin_text
    assert str(env_path) not in admin_text
    assert "全部高级参数" in advanced_text
    assert "FLAGO_SQLITE_PATH" in advanced_text
    assert "clear-button" in advanced_text
    saved_env = env_path.read_text(encoding="utf-8")
    assert "FLAGO_BASE_URL=https://flago.example.test" in saved_env
    assert "FLAGO_AGENT_MODE=agent" in saved_env
    assert "FLAGO_DEFAULT_PROVIDER=gemini" in saved_env
    assert "FLAGO_WRITEBACK_ENABLED=true" in saved_env
    assert "FLAGO_WRITEBACK_CONFIRMATION_MODE=low_risk_direct" in saved_env
...
def test_local_setup_bootstrap_allows_first_run_without_feishu_login(tmp_path) -> None:
    ...
    settings = _settings(tmp_path).model_copy(
        update={
            "admin_config_path": env_path,
            "base_url": "https://flago.example.test",
        }
    )
            ...
            content=urlencode(
                {
                    ...
                    "next": "/admin/setup",
                    "config_scope": "setup",
                    "env__FLAGO_BASE_URL": "http://127.0.0.1:8000",
                    "env__FLAGO_ENV": "dev",
                    "env__FLAGO_AGENT_MODE": "agent",
                }
            ),
    ...
    assert "https://flago.example.test" not in setup.text
    ...
    assert "FLAGO_BASE_URL=http://127.0.0.1:8000" in env_path.read_text(
        encoding="utf-8"
    )
