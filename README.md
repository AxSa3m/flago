...
- Gemini、Anthropic、OpenAI 兼容模型等多模型配置。
...
- 已获得使用相关飞书文档、消息、表格、多维表、文件和企业数据的合法权限。
- 第三方模型、API、代理服务和工作流服务的使用符合其服务条款。
- 输入、上传、处理、存储和导出的数据符合所在组织、地区和行业的数据安全、隐私和合规要求。
- 妥善保管 API Key、App Secret、OAuth Token、代理地址和其他凭据。

本项目不提供第三方服务本身，也不对用户配置的第三方服务、权限范围、数据处理行为或由此产生的费用、数据泄露、合规风险承担责任。
