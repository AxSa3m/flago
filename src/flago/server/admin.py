...
from flago.local_service import service_status, trigger_service_action
from flago.logging import redact
from flago.model_providers.registry import build_model_router
from flago.model_providers.types import ModelMessage, ModelMessageRole, ModelRequest
...
MENU_GROUPS: tuple[tuple[str, tuple[tuple[str, str], ...]], ...] = (
    ...
    (
        "模型",
        (
            ("flago.model.view", "查看当前模型"),
            ("flago.model.default", "恢复默认模型"),
    ...
    (
        "授权",
        (
            ("flago.auth.start", "飞书授权"),
            ("flago.auth.status", "授权状态"),
        ),
    ),
    (
        "上下文",
    ...
    (
        "写入",
        (
            ("flago.writeback.status", "写入状态"),
            ("flago.writeback.auto.enable", "自动写入开启"),
            ("flago.writeback.auto.disable", "自动写入关闭"),
            ("flago.writeback.auto.clear", "写入恢复默认"),
            ("flago.writeback.history", "最近写入"),
    ...
    (
        "记忆",
        (
            ("flago.memory.view", "查看记忆"),
            ("flago.memory.delete", "删除记忆，需要确认"),
            ("flago.memory.disable", "关闭记忆"),
            ("flago.memory.enable", "开启记忆"),
        ),
    ),
    (
        "帮助",
...
COMMON_ENV_NAMES: set[str] = {
    ...
    "FLAGO_ATTACHMENT_OCR_ENABLED",
    "FLAGO_ATTACHMENT_VISION_ENABLED",
    "FLAGO_ATTACHMENT_MEDIA_UNDERSTANDING_ENABLED",
    "FLAGO_WEB_READ_ENABLED",
    "FLAGO_WRITEBACK_ENABLED",
    "FLAGO_WRITEBACK_AUTO_EXECUTE_ENABLED",
...
ADMIN_EXCLUDED_ENV_NAMES: set[str] = {"FLAGO_ADMIN_ENABLED"}

WRITE_ONLY_ENV_NAMES: set[str] = {
    "FEISHU_APP_ID",
    "FEISHU_BOT_OPEN_ID",
    "FLAGO_ADMIN_CONFIG_PATH",
}
...
FIELD_METADATA: dict[str, tuple[str, str]] = {
    ...
    "FLAGO_AGENT_MODE": ("Agent 模式", "legacy 使用旧解析链路；agent 使用 Agent + Tools 链路。"),
    ...
    "FLAGO_DEFAULT_PROVIDER": ("系统默认 Provider", "没有个人偏好时使用的模型 Provider。"),
    "FLAGO_DEFAULT_MODEL": ("系统默认模型", "留空时使用所选 Provider 的模型配置。"),
    "FLAGO_RESOURCE_SEARCH_ENABLED": (
        "飞书资源搜索",
        "允许按名称或关键词搜索飞书文档、表格和多维表。",
    ),
    "FLAGO_ATTACHMENT_OCR_ENABLED": ("图片 OCR", "用本地 OCR 工具提取图片文字。"),
    "FLAGO_ATTACHMENT_VISION_ENABLED": ("图片理解", "用多模态模型理解图片内容。"),
    "FLAGO_ATTACHMENT_MEDIA_UNDERSTANDING_ENABLED": (
        "音视频理解",
        "用多模态模型理解音频或视频内容。",
    ),
    "FLAGO_WEB_READ_ENABLED": ("网页浏览", "允许读取用户提供的网页链接。"),
    "FLAGO_WRITEBACK_ENABLED": ("写入功能", "允许生成并执行写入确认卡片。"),
    "FLAGO_WRITEBACK_AUTO_EXECUTE_ENABLED": (
        "自动写入总开关",
    ...
    *,
    ...
    async def admin_home(request: Request) -> Response:
        if not settings.admin_enabled:
            return HTMLResponse(
                _message_page("本地配置后台未启用", "请检查 FLAGO_ADMIN_ENABLED。"),
                404,
            )
    ...
    async def admin_set_default_model_provider(request: Request) -> RedirectResponse:
        ...
        updates: dict[str, str | None] = {
            "FLAGO_DEFAULT_PROVIDER": provider.key,
            "FLAGO_DEFAULT_MODEL": provider.model or None,
        }
...
def _is_example_base_url(value: str) -> bool:
    ...
    return host in {"flago.example.test", "flago.example.com"} or host.endswith(".example.test")
...
def _config_scope_env_names(scope: str) -> set[str] | None:
    ...
    if scope == "assistant":
        return set()
    if scope == "basic":
        return {"FLAGO_ENV", "FLAGO_LOG_LEVEL", "FLAGO_BASE_URL", "FLAGO_AGENT_MODE"}
    if scope == "feishu":
        return {
            "FEISHU_APP_ID",
            "FEISHU_APP_SECRET",
    ...
    if scope == "model":
        env_names = {"FLAGO_DEFAULT_PROVIDER", "FLAGO_DEFAULT_MODEL"}
...
def _humanize_env_name(env_name: str) -> str:
    cleaned = env_name
    for prefix in ("FLAGO_", "FEISHU_", "GEMINI_", "OPENAI_", "DEEPSEEK_"):
...
def _setting_group(env_name: str) -> str:
    if env_name.startswith(("FEISHU_", "FLAGO_OAUTH")):
        return "飞书应用"
    ...
    if env_name.startswith(
        ...
        return "资源与附件读取"
    if env_name.startswith("FLAGO_WEB_"):
        return "网页读取"
    if env_name.startswith(("FLAGO_CONTEXT_", "FLAGO_MEMORY_")):
        return "上下文与记忆"
    if env_name.startswith(
        (
            "FLAGO_WRITEBACK_",
            "FLAGO_PENDING_",
            "FLAGO_CARD_",
            "FLAGO_MAX_WRITEBACK_",
        )
    ):
        return "写入"
    if env_name.startswith("FLAGO_AGENT_"):
        return "Agent"
    if env_name.startswith("FLAGO_ADMIN_"):
        return "后台"
    return "基础服务"
          ...
          <section class="wide" data-service-control>
            <article>
              ...
              <p class="hint">
                ...
                <code>uv run flago service start --open-admin</code> 使用。
              </p>
...
async def _setup_admin_page(
    ...
    *,
    ...
    configured_models = _configured_provider_cards(
        provider_options,
        system_default_provider=_specs_by_env(setup_settings)["FLAGO_DEFAULT_PROVIDER"].value,
          ...
          <section class="setup-shell" data-setup-wizard>
            ...
            <article class="setup-step" data-setup-step="1" hidden>
              ...
              <div class="settings-list one-column">
                {_setting_fields_for(setup_settings, ("FLAGO_BASE_URL",))}
              </div>
              <p class="hint">
            ...
            <article class="setup-step" data-setup-step="2" hidden>
              ...
              <p>
                ...
                再回到这里填写应用凭证。Flago只会连接你填入的这个应用，不会默认使用任何人的机器人。
              </p>
              <div class="settings-list one-column">
                {_setting_fields_for(
                    setup_settings,
            ...
            <article class="setup-step" data-setup-step="4" hidden>
              ...
              <div class="setup-menu-minimal">
                <div><strong>后台配置</strong><code>flago.admin.open</code></div>
                <div><strong>使用说明</strong><code>flago.help</code></div>
                <div><strong>授权状态</strong><code>flago.auth.status</code></div>
              </div>
...
def _setup_status_cards(settings: Settings, providers: list[ModelProviderOption]) -> str:
    ...
    base_ready = bool(specs["FLAGO_BASE_URL"].value.strip())
    model_ready = any(provider.configured for provider in providers)
    default_provider = specs["FLAGO_DEFAULT_PROVIDER"].value
    default_ready = any(
...
def _basic_settings_card(settings: Settings) -> str:
    ...
    <article>
      ...
      <div class="settings-list one-column">
        {_setting_fields_for(
            settings,
            ("FLAGO_ENV", "FLAGO_LOG_LEVEL", "FLAGO_BASE_URL", "FLAGO_AGENT_MODE"),
        )}
      </div>
      {_card_actions("basic")}
    </article>
...
def _model_interfaces_card(
    ...
    system_default = _specs_by_env(settings)["FLAGO_DEFAULT_PROVIDER"].value
...
def _system_default_model_fields(
    ...
    specs = _specs_by_env(settings)
    selected_provider = specs["FLAGO_DEFAULT_PROVIDER"].value
    selected_model = specs["FLAGO_DEFAULT_MODEL"].value
    ...
    <label class="setting-field">
      {_field_meta(specs["FLAGO_DEFAULT_PROVIDER"])}
      <select
        name="{escape(_env_input_name("FLAGO_DEFAULT_PROVIDER"))}"
        id="system-provider"
        {disabled}
      >
        {provider_options}
    ...
    <label class="setting-field">
      {_field_meta(specs["FLAGO_DEFAULT_MODEL"])}
      <select
        name="{escape(_env_input_name("FLAGO_DEFAULT_MODEL"))}"
        id="system-model"
...
def _model_options_script(providers: list[ModelProviderOption]) -> str:
    ...
    return f"""
    <script>
      const flagoModelsByProvider = {data};
      function refreshModelSelect(providerId, modelId) {{
        ...
        const models = flagoModelsByProvider[provider] || [];
...
def _setup_wizard_script() -> str:
    ...
    <script>
      ...
      if (setupWizard) {
        ...
        const baseUrlInput = setupWizard.querySelector('[name="env__FLAGO_BASE_URL"]');
...
def _choice_help(env_name: str) -> str:
    ...
    if env_name == "FLAGO_WRITEBACK_CONFIRMATION_MODE":
        return (
            '<ul class="choice-help">'
