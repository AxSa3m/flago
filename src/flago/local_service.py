...
def service_status(
    *,
    ...
    if health:
        ...
        state = "blocked"
        message = f"端口 {service_port} 已被其他程序占用，请关闭占用程序或修改 FLAGO_PORT"
    else:
        state = "stopped"
        message = "服务未运行"
    return {
...
def stop_service(workspace: Path | None = None) -> list[int]:
    pids = flago_server_pids(workspace, exclude_current=True)
    for pid in pids:
        _kill_process(pid)
...
def flago_server_pids(
    workspace: Path | None = None,
    *,
    exclude_current: bool = True,
) -> list[int]:
    if os.name == "nt":
        return _windows_flago_server_pids(
            workspace,
            exclude_pid=os.getpid() if exclude_current else None,
        )
    return _posix_flago_server_pids()
...
def _windows_flago_server_pids(
    workspace: Path | None = None,
    *,
    ...
    command = (
        ...
        "$_.CommandLine -like '*-m flago.cli serve*' } | "
...
def _windows_python_pids_using_workspace_modules(workspace: Path) -> list[int]:
    ...
    command = (
        ...
        "if ($cmd -like '*spawn_main*' -or $cmd -like '*-m flago.cli serve*') { "
        "try { "
...
def _posix_flago_server_pids() -> list[int]:
    completed = subprocess.run(
        ["pgrep", "-f", "--", "-m flago.cli serve"],
        check=False,
        capture_output=True,
        text=True,
    )
...
def _wait_until_stopped(workspace: Path | None = None) -> None:
    ...
    while time.monotonic() < deadline:
        if not flago_server_pids(workspace, exclude_current=True):
            return
        time.sleep(0.3)
    raise RuntimeError("old flago server process did not stop within 10 seconds")
...
def _wait_until_healthy(
    ...
    while time.monotonic() < deadline:
        if _health_url_ok(health_url):
            return
        if proc.poll() is not None and not flago_server_pids():
            raise RuntimeError(f"flago server exited early with code {proc.returncode}")
        time.sleep(0.8)
    if _health_url_ok(health_url):
        return
    raise RuntimeError(f"flago server did not become healthy at {health_url}")
