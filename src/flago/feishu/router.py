...
from flago.feishu.context import ChatContextLoader, ChatHistoryAPI, conversation_memory_source
from flago.feishu.oauth import AuthorizationStatus
from flago.logging import redact
from flago.model_providers.catalog import find_catalog_item
from flago.model_providers.registry import ModelRouter
...
from flago.writeback.cards import assistant_response_card
...
class FeishuMessageRouter:
    ...
    async def _handle_menu_action(self, event: FeishuBotMenuEvent) -> str:
        ...
        if key.startswith("flago.model.use."):
            provider = key.removeprefix("flago.model.use.")
            return await self._set_menu_user_model(event, provider, assistant_name=assistant_name)
        if key in {"flago.assistant.name.view", "flago.assistant.info.view"}:
        ...
        *,
        ...
        if not self.settings.writeback_auto_execute_enabled:
            return (
                ...
                "`FLAGO_WRITEBACK_AUTO_EXECUTE_ENABLED=true` 后才能开启。"
            )
...
def _normalize_menu_key(event_key: str) -> str:
    ...
    if key.startswith("flago."):
        return "flago." + key.removeprefix("flago.")
    return key
            ...
            },
            {
                ...
                "actions": [
                    ...
                    },
                    {
                        ...
                        "value": {
                            "action": "cancel",
                            "flago_action": "flago.memory.save.cancel",
                            "action_id": action_id,
                        },
                    },
                ],
            ...
            },
            {
                ...
                "actions": [
                    ...
                    },
                    {
                        ...
                        "value": {
                            "action": "cancel",
                            "flago_action": "flago.memory.delete.cancel",
                            "action_id": action_id,
                        },
                    },
                ],
