...
from flago.agent.protocols import (
    AuditRecorder,
    ModelProvider,
    ResourceReader,
    ResourceSearcher,
...
from flago.resources.parser import parse_resource_urls
...
def _model_writeback_json_objects(text: str) -> list[dict[str, Any]]:
    ...
    for match in re.finditer(fenced_pattern, text, flags=re.DOTALL):
        ...
        if "flago_writeback" not in block and "FLAGO_WRITEBACK_PLAN" not in block:
            continue
        parsed = _json_object_from_text(block)
        if parsed is not None:
            objects.append(parsed)
    marker_match = re.search(
        r"FLAGO_WRITEBACK_PLAN\s*[:：]\s*(\{.*\})",
        text,
        flags=re.DOTALL,
    )
    if marker_match:
...
def _json_object_from_text(text: str) -> dict[str, Any] | None:
    ...
    if candidate.startswith("FLAGO_WRITEBACK_PLAN"):
...
def _plans_from_json_object(obj: dict[str, Any]) -> list[_ModelWritebackPlan]:
    raw: Any
    if "flago_writeback" in obj:
        raw = obj["flago_writeback"]
    elif "flago_writeback_plan" in obj:
        raw = obj["flago_writeback_plan"]
...
def _remove_model_writeback_plan_blocks(text: str) -> str:
    cleaned = re.sub(
        r"```(?:json|JSON)?\s*.*?flago_writeback.*?```",
        "",
        text,
        flags=re.DOTALL,
    )
    cleaned = re.sub(
        r"FLAGO_WRITEBACK_PLAN\s*[:：]\s*\{.*\}\s*$",
        "",
        cleaned,
        flags=re.DOTALL,
    )
