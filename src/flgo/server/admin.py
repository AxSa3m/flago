...
from flgo.feishu.oauth import FeishuOAuthService
from flgo.local_service import service_status, trigger_service_action
from flgo.logging import redact
from flgo.model_providers.registry import build_model_router
from flgo.model_providers.types import ModelMessage, ModelMessageRole, ModelRequest
...
COMMON_ENV_NAMES: set[str] = {
    ...
    "FEISHU_BOT_OPEN_ID",
    "FEISHU_BOT_NAME",
    "FLGO_OAUTH_ENABLE_OFFLINE_ACCESS",
    "FLGO_DEFAULT_PROVIDER",
    "FLGO_DEFAULT_MODEL",
    "GEMINI_API_KEY",
    "GEMINI_DISPLAY_NAME",
    "GEMINI_BASE_URL",
    ...
    "ANTHROPIC_BASE_URL",
    "ANTHROPIC_MODEL",
    "FLGO_RESOURCE_SEARCH_ENABLED",
    "FLGO_ATTACHMENT_OCR_ENABLED",
    "FLGO_ATTACHMENT_VISION_ENABLED",
    "FLGO_ATTACHMENT_MEDIA_UNDERSTANDING_ENABLED",
    "FLGO_WEB_READ_ENABLED",
    "FLGO_WRITEBACK_ENABLED",
    "FLGO_WRITEBACK_AUTO_EXECUTE_ENABLED",
    "FLGO_WRITEBACK_CONFIRMATION_MODE",
    "SEEDANCE_API_KEY",
...
ADMIN_EXCLUDED_ENV_NAMES: set[str] = {"FLGO_ADMIN_ENABLED"}

WRITE_ONLY_ENV_NAMES: set[str] = {
    "FEISHU_APP_ID",
    "FEISHU_BOT_OPEN_ID",
    "FLGO_ADMIN_CONFIG_PATH",
}
...
FIELD_METADATA: dict[str, tuple[str, str]] = {
    ...
    "FLGO_AGENT_MODE": ("Agent 模式", "legacy 使用旧解析链路；agent 使用 Agent + Tools 链路。"),
    ...
    "FLGO_DEFAULT_PROVIDER": ("系统默认 Provider", "没有个人偏好时使用的模型 Provider。"),
    "FLGO_DEFAULT_MODEL": ("系统默认模型", "留空时使用所选 Provider 的模型配置。"),
    "FLGO_RESOURCE_SEARCH_ENABLED": (
        "飞书资源搜索",
        "允许按名称或关键词搜索飞书文档、表格和多维表。",
    ),
    "FLGO_ATTACHMENT_OCR_ENABLED": ("图片 OCR", "用本地 OCR 工具提取图片文字。"),
    "FLGO_ATTACHMENT_VISION_ENABLED": ("图片理解", "用多模态模型理解图片内容。"),
    "FLGO_ATTACHMENT_MEDIA_UNDERSTANDING_ENABLED": (
        "音视频理解",
        "用多模态模型理解音频或视频内容。",
    ),
    "FLGO_WEB_READ_ENABLED": ("网页浏览", "允许读取用户提供的网页链接。"),
    "FLGO_WRITEBACK_ENABLED": ("写入功能", "允许生成并执行写入确认卡片。"),
    "FLGO_WRITEBACK_AUTO_EXECUTE_ENABLED": (
        "自动写入总开关",
    ...
    *,
    ...
    async def admin_home(request: Request) -> Response:
        if not settings.admin_enabled:
            return HTMLResponse(
                _message_page("本地配置后台未启用", "请检查 FLGO_ADMIN_ENABLED。"),
                404,
            )
    ...
    async def admin_set_default_model_provider(request: Request) -> RedirectResponse:
        ...
        updates: dict[str, str | None] = {
            "FLGO_DEFAULT_PROVIDER": provider.key,
            "FLGO_DEFAULT_MODEL": provider.model or None,
        }
...
def _is_example_base_url(value: str) -> bool:
    ...
    return host in {"flgo.example.test", "flgo.example.com"} or host.endswith(".example.test")
...
def _config_scope_env_names(scope: str) -> set[str] | None:
    ...
    if scope == "assistant":
        return set()
    if scope == "basic":
        return {"FLGO_ENV", "FLGO_LOG_LEVEL", "FLGO_BASE_URL", "FLGO_AGENT_MODE"}
    if scope == "feishu":
        return {
            "FEISHU_APP_ID",
            "FEISHU_APP_SECRET",
    ...
    if scope == "model":
        env_names = {"FLGO_DEFAULT_PROVIDER", "FLGO_DEFAULT_MODEL"}
...
def _humanize_env_name(env_name: str) -> str:
    cleaned = env_name
    for prefix in ("FLGO_", "FEISHU_", "GEMINI_", "OPENAI_", "DEEPSEEK_"):
...
def _setting_group(env_name: str) -> str:
    if env_name.startswith(("FEISHU_", "FLGO_OAUTH")):
        return "飞书应用"
    ...
    if env_name.startswith(
        ...
        return "资源与附件读取"
    if env_name.startswith("FLGO_WEB_"):
        return "网页读取"
    if env_name.startswith(("FLGO_CONTEXT_", "FLGO_MEMORY_")):
        return "上下文与记忆"
    if env_name.startswith(
        (
            "FLGO_WRITEBACK_",
        ...
        return "写入"
    if env_name.startswith("FLGO_AGENT_"):
        return "Agent"
    if env_name.startswith("FLGO_ADMIN_"):
        return "后台"
    return "基础服务"
...
async def _setup_admin_page(
    ...
    *,
    ...
    configured_models = _configured_provider_cards(
        provider_options,
        system_default_provider=_specs_by_env(setup_settings)["FLGO_DEFAULT_PROVIDER"].value,
          ...
          <section class="setup-shell" data-setup-wizard>
            ...
            <article class="setup-step" data-setup-step="1" hidden>
              ...
              <div class="settings-list one-column">
                {_setting_fields_for(setup_settings, ("FLGO_BASE_URL",))}
              </div>
              <p class="hint">
...
def _setup_status_cards(settings: Settings, providers: list[ModelProviderOption]) -> str:
    ...
    base_ready = bool(specs["FLGO_BASE_URL"].value.strip())
    model_ready = any(provider.configured for provider in providers)
    default_provider = specs["FLGO_DEFAULT_PROVIDER"].value
    default_ready = any(
...
def _basic_settings_card(settings: Settings) -> str:
    ...
    <article>
      ...
      <div class="settings-list one-column">
        {_setting_fields_for(
            settings,
            ("FLGO_ENV", "FLGO_LOG_LEVEL", "FLGO_BASE_URL", "FLGO_AGENT_MODE"),
        )}
      </div>
      {_card_actions("basic")}
    </article>
...
def _model_interfaces_card(
    ...
    system_default = _specs_by_env(settings)["FLGO_DEFAULT_PROVIDER"].value
...
def _system_default_model_fields(
    ...
    specs = _specs_by_env(settings)
    selected_provider = specs["FLGO_DEFAULT_PROVIDER"].value
    selected_model = specs["FLGO_DEFAULT_MODEL"].value
    ...
    <label class="setting-field">
      {_field_meta(specs["FLGO_DEFAULT_PROVIDER"])}
      <select
        name="{escape(_env_input_name("FLGO_DEFAULT_PROVIDER"))}"
        id="system-provider"
        {disabled}
      >
        {provider_options}
    ...
    <label class="setting-field">
      {_field_meta(specs["FLGO_DEFAULT_MODEL"])}
      <select
        name="{escape(_env_input_name("FLGO_DEFAULT_MODEL"))}"
        id="system-model"
...
def _model_options_script(providers: list[ModelProviderOption]) -> str:
    ...
    return f"""
    <script>
      const flgoModelsByProvider = {data};
      function refreshModelSelect(providerId, modelId) {{
        ...
        const models = flgoModelsByProvider[provider] || [];
...
def _setup_wizard_script() -> str:
    ...
    <script>
      ...
      if (setupWizard) {
        ...
        const baseUrlInput = setupWizard.querySelector('[name="env__FLGO_BASE_URL"]');
...
def _choice_help(env_name: str) -> str:
    ...
    if env_name == "FLGO_WRITEBACK_CONFIRMATION_MODE":
        return (
            '<ul class="choice-help">'
