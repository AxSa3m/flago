...
def main() -> None:
    ...
    if args.command == "service":
        from flgo.local_service import run_service_action
    ...
    if args.command == "package":
        ...
        from flgo.packaging import PackageTarget, build_portable_package, current_target
    ...
    from flgo.logging import configure_logging
...
def _run_worker() -> None:
    from flgo.agent import AgentOrchestrator, Assistant, default_tool_registry
    from flgo.agent.protocols import AssistantHandler
    from flgo.agent.search_planner import ModelResourceSearchPlanner
    from flgo.config import get_settings
    ...
    from flgo.feishu.worker import FeishuLongConnectionWorker
    from flgo.resources.reader import (
...
async def _doctor() -> bool:
    ...
    for name, passed, detail in checks:
        ...
        if not passed:
            print(f"      {detail}")
    print(f"[INFO] FLGO_START_LONG_CONNECTION={settings.start_long_connection}")
