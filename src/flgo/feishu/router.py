...
from flgo.agent.protocols import AssistantHandler
from flgo.config import Settings
from flgo.feishu.client import FeishuClient
from flgo.feishu.context import ChatContextLoader, ChatHistoryAPI, conversation_memory_source
from flgo.feishu.oauth import AuthorizationStatus
from flgo.logging import redact
from flgo.model_providers.catalog import find_catalog_item
from flgo.model_providers.registry import ModelRouter
...
from flgo.writeback.cards import assistant_response_card
...
class FeishuMessageRouter:
    ...
    async def _handle_menu_action(self, event: FeishuBotMenuEvent) -> str:
        ...
        if key.startswith("flgo.model.use."):
            provider = key.removeprefix("flgo.model.use.")
            return await self._set_menu_user_model(event, provider, assistant_name=assistant_name)
        if key in {"flgo.assistant.name.view", "flgo.assistant.info.view"}:
        ...
        *,
        ...
        if not self.settings.writeback_auto_execute_enabled:
            return (
                ...
                "`FLGO_WRITEBACK_AUTO_EXECUTE_ENABLED=true` 后才能开启。"
            )
...
def _normalize_menu_key(event_key: str) -> str:
    ...
    if key.startswith("flgo."):
        return "flgo." + key.removeprefix("flgo.")
    return key
