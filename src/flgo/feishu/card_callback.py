...
def parse_card_callback(payload: dict[str, Any]) -> FeishuCardCallback:
    ...
    raw_action = value.get("flgo_action") or value.get("action") or ""
...
def _normalize_action(action: str) -> str:
    ...
    if normalized in {
        "confirm",
        "writeback.confirm",
        "flgo.writeback.confirm",
        "memory.delete.confirm",
        "flgo.memory.delete.confirm",
        "memory.save.confirm",
        "flgo.memory.save.confirm",
    }:
        return "confirm"
    if normalized in {
        "cancel",
        "writeback.cancel",
        "flgo.writeback.cancel",
        "memory.delete.cancel",
        "flgo.memory.delete.cancel",
        "memory.save.cancel",
        "flgo.memory.save.cancel",
    }:
        return "cancel"
    return normalized
