...
解压 `fcgo-portable-windows.zip` 后，双击：
...
服务地址不是机器人聊天消息入口。机器人聊天消息通过飞书长连接进入 FCGO。
...
FCGO 不会自动帮你创建飞书应用，也不会默认连接作者或其他人的机器人。你在后台填写哪个
...
模型
- 查看模型              fcgo.model.view
- 恢复默认模型          fcgo.model.default
- 使用 Gemini           fcgo.model.use.gemini
- 使用 DeepSeek         fcgo.model.use.deepseek
- 使用 OpenAI           fcgo.model.use.openai
- 使用 Qwen             fcgo.model.use.qwen
- 使用 Doubao           fcgo.model.use.doubao
- 使用 Minimax          fcgo.model.use.minimax
- 使用 Claude           fcgo.model.use.claude
...
```text
fcgo.assistant.info.view
fcgo.context.view
fcgo.context.enable
fcgo.context.disable
```
...
如果你使用便携包，通常双击 `start-fcgo.cmd` 就够了。
