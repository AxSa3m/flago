...
- `FCGO_DEFAULT_PROVIDER`：部署默认 Provider。
- `FCGO_DEFAULT_MODEL`：可选，覆盖 Provider 默认模型。
- `FCGO_MODEL_TEST_MAX_OUTPUT_TOKENS`：测试环境默认输出 token 上限。
- `FCGO_MODEL_PROVIDER_CONCURRENCY_LIMIT`：每个 Provider 的并发调用上限，`0` 表示关闭路由层并发限制。

飞书中的“查看模型”只显示当前实际使用的模型、可用模型和未启用的 Provider 名称。
密钥缺失等部署细节不向普通用户展开；部署者可通过 `fcgo doctor` 和服务日志检查配置。
...
`GEMINI_MAX_OUTPUT_TOKENS` 不设置时，dev/prod 不硬限制输出 token；test 环境使用 `FCGO_MODEL_TEST_MAX_OUTPUT_TOKENS`。

## OpenAI

```env
FCGO_DEFAULT_PROVIDER=openai
OPENAI_API_KEY=
OPENAI_BASE_URL=https://api.openai.com/v1
OPENAI_MODEL=
FCGO_OPENAI_COMPATIBLE_TIMEOUT_SECONDS=60
# FCGO_OPENAI_COMPATIBLE_MAX_OUTPUT_TOKENS=4096
