...
- `FLAGO_DEFAULT_PROVIDER` / `FLAGO_DEFAULT_MODEL`：可选；用于选择部署默认 Provider 和模型
- OpenAI 兼容 Provider：如需使用 OpenAI、DeepSeek、Qwen、Doubao 或 Minimax，填入对应
  `API_KEY`、`BASE_URL`、`MODEL`
- `FLAGO_BASE_URL`
- `FLAGO_START_LONG_CONNECTION=true`
- `FLAGO_AGENT_MODE=legacy`：默认保持当前稳定编排；测试 Agent + Tools 时改为 `agent`
- `FLAGO_WRITEBACK_ENABLED=false`：默认关闭写回；测试确认式写回时改为 `true`
- `FLAGO_WRITEBACK_AUTO_EXECUTE_ENABLED=false`：默认不允许用户开启低风险自动执行；需要该能力时改为 `true`
- `FLAGO_WRITEBACK_CONFIRMATION_MODE=always`：写回确认策略，可选 `always`（每次确认）、`low_risk_direct`（低风险自动执行）、`draft_only`（仅生成草稿）

最小 Gemini 直连配置示例：

```env
FLAGO_ENV=prod
FLAGO_BASE_URL=https://flago.example.com
FLAGO_START_LONG_CONNECTION=true
FLAGO_DEFAULT_PROVIDER=gemini
FLAGO_AGENT_MODE=legacy
...
FLAGO_WRITEBACK_ENABLED=false
FLAGO_WRITEBACK_AUTO_EXECUTE_ENABLED=false
FLAGO_WRITEBACK_CONFIRMATION_MODE=always
```

`FLAGO_BASE_URL` 必须填写部署服务的外部访问根地址，不要包含
...
当 `FLAGO_DEFAULT_PROVIDER=gemini` 时，doctor 会执行一个轻量 Gemini 请求。只有默认直连报告
...
- 如需自动刷新用户 access token，需要在飞书开发者后台开通 `offline_access`，然后设置 `FLAGO_OAUTH_ENABLE_OFFLINE_ACCESS=true`。未开通时不要开启，否则飞书授权页会报 `20027` 应用权限不足。
- 机器人授权回调地址：`{FLAGO_BASE_URL}/oauth/feishu/callback`
- 本地配置后台登录回调地址：`{FLAGO_BASE_URL}/admin/oauth/callback`

写回和记忆确认卡片均通过长连接接收卡片操作。启用写回时设置
`FLAGO_WRITEBACK_ENABLED=true`；关闭时不会创建或执行写回。
...
- 本地测试：`FLAGO_BASE_URL=http://127.0.0.1:8000`。
- 服务器部署：将 Flago（FLAGO） 部署到带 HTTPS 域名的服务器。
- 隧道或反向代理：使用公网 HTTPS 域名转发到 `127.0.0.1:8000`。

例如外部地址是 `https://flago.example.com`：
...
```text
https://flago.example.com/oauth/feishu/callback
https://flago.example.com/admin/oauth/callback
```
...
Flago（FLAGO） 会通过 open_id 给操作者发送结果；模型菜单设置的是该用户的个人默认模型。
文本对话不会触发模型切换；`/模型 查看` 仅用于查看当前配置。
记忆菜单作用于点击菜单的用户，不会查看或修改其他用户的长期记忆。
菜单里的 `flago.memory.delete` 会发送清空全部长期记忆的确认卡片；按 key 删除单条记忆请使用文本命令。
写入菜单作用于点击菜单的用户；自动写入只影响该用户个人偏好，不会影响其他用户。
帮助菜单里的 `flago.admin.open` 会发送本地配置后台链接。首次打开 `/admin` 需要使用飞书登录；
...
- 授权 state 默认 10 分钟过期，可通过 `FLAGO_OAUTH_STATE_TTL_SECONDS` 调整。
...
FLAGO_RESOURCE_SEARCH_ENABLED=true
FLAGO_RESOURCE_SEARCH_RESULT_LIMIT=5
FLAGO_RESOURCE_SEARCH_READ_LIMIT=3
FLAGO_MAX_SHEET_ROWS=200
FLAGO_DOC_BLOCK_SCAN_LIMIT=1000
FLAGO_EMBEDDED_FILE_LIMIT=3
FLAGO_EMBEDDED_FILE_MAX_BYTES=20971520
FLAGO_EMBEDDED_FILE_MAX_CHARS=40000
FLAGO_ATTACHMENT_OCR_ENABLED=false
FLAGO_ATTACHMENT_OCR_COMMAND=tesseract
FLAGO_ATTACHMENT_OCR_LANGUAGES=chi_sim+eng
FLAGO_ATTACHMENT_OCR_TIMEOUT_SECONDS=15
FLAGO_ATTACHMENT_OCR_MAX_PIXELS=20000000
FLAGO_ATTACHMENT_VISION_ENABLED=false
FLAGO_ATTACHMENT_VISION_PROVIDER=gemini
FLAGO_ATTACHMENT_VISION_MODEL=
FLAGO_ATTACHMENT_VISION_MAX_BYTES=5242880
FLAGO_ATTACHMENT_VISION_MAX_PIXELS=20000000
FLAGO_ATTACHMENT_MEDIA_UNDERSTANDING_ENABLED=false
FLAGO_ATTACHMENT_MEDIA_UNDERSTANDING_PROVIDER=gemini
FLAGO_ATTACHMENT_MEDIA_UNDERSTANDING_MODEL=
FLAGO_ATTACHMENT_MEDIA_UNDERSTANDING_MAX_BYTES=20971520
FLAGO_EMBEDDED_LINK_LIMIT=20
FLAGO_PDF_DEFAULT_PAGES=2
FLAGO_PDF_MAX_PAGES=10
FLAGO_PDF_EXTRACT_TIMEOUT_SECONDS=20
FLAGO_MAX_SHEET_COLUMNS=26
FLAGO_WEB_READ_ENABLED=true
FLAGO_WEB_TIMEOUT_SECONDS=20
FLAGO_WEB_MAX_BYTES=1000000
FLAGO_WEB_ALLOWED_HOSTS=
FLAGO_WEB_BLOCKED_HOSTS=
```

内嵌对象读取说明：

- `FLAGO_EMBEDDED_FILE_LIMIT` 限制单篇文档下载和检查的文件/图片数量。
- `FLAGO_EMBEDDED_FILE_MAX_BYTES` 限制单个素材下载体积。
- `FLAGO_EMBEDDED_FILE_MAX_CHARS` 限制单个附件注入模型上下文的文本长度。
- `FLAGO_ATTACHMENT_OCR_ENABLED` 默认关闭。设为 `true` 后，图片附件会调用本机
  `FLAGO_ATTACHMENT_OCR_COMMAND` 指向的 Tesseract 命令做 OCR；如果本机未安装或超时，会保留图片元数据并返回原因。
- `FLAGO_ATTACHMENT_OCR_LANGUAGES` 默认 `chi_sim+eng`，需要本机 Tesseract 已安装对应语言包。
- `FLAGO_ATTACHMENT_OCR_TIMEOUT_SECONDS` 和 `FLAGO_ATTACHMENT_OCR_MAX_PIXELS` 用于限制 OCR 成本和大图风险。
- `FLAGO_ATTACHMENT_VISION_ENABLED` 默认关闭。设为 `true` 后，图片附件会优先发送给
  `FLAGO_ATTACHMENT_VISION_PROVIDER` 指定的支持视觉输入的模型做图片理解；失败时回退到 OCR 或图片元数据。
- `FLAGO_ATTACHMENT_VISION_MODEL` 可指定视觉模型，留空则使用该 Provider 默认模型。
- `FLAGO_ATTACHMENT_VISION_MAX_BYTES` 和 `FLAGO_ATTACHMENT_VISION_MAX_PIXELS` 用于限制图片发送到模型的体积和尺寸。
- `FLAGO_ATTACHMENT_MEDIA_UNDERSTANDING_ENABLED` 默认关闭。设为 `true` 后，音频和视频附件会优先发送给
  `FLAGO_ATTACHMENT_MEDIA_UNDERSTANDING_PROVIDER` 指定的多模态模型做转写或内容理解；失败时回退到安全元数据说明。
- `FLAGO_ATTACHMENT_MEDIA_UNDERSTANDING_MODEL` 可指定媒体理解模型，留空则使用该 Provider 默认模型。
- `FLAGO_ATTACHMENT_MEDIA_UNDERSTANDING_MAX_BYTES` 用于限制音视频发送到模型的体积。
- `FLAGO_EMBEDDED_LINK_LIMIT` 限制单篇文档列出的超链接、`@文档` 和内嵌网页数量。
- Office Open XML 文件在解析前检查压缩包条目数和解压后体积，避免压缩炸弹。
- 可执行文件不会执行，压缩包不会解压，脚本文件只按纯文本读取。

私聊中没有显式链接、且用户明确要求搜索/读取飞书文档、表格、多维表、知识库或资料时，Flago（FLAGO） 会按用户 OAuth
...
`FLAGO_WEB_READ_ENABLED=false` 可关闭普通网页读取。开启时会按 `FLAGO_WEB_TIMEOUT_SECONDS` 抓取，并只读取
`text/html`、`text/plain` 和 `application/xhtml+xml` 这类可读文本内容；本机、内网、保留地址、二进制或
超过 `FLAGO_WEB_MAX_BYTES` 的内容会被安全跳过。`FLAGO_WEB_ALLOWED_HOSTS` 可限制只读指定域名，
`FLAGO_WEB_BLOCKED_HOSTS` 可禁止指定域名；多个域名可用空格或逗号分隔。
...
`FLAGO_CONTEXT_INJECT_MESSAGE_LIMIT` 和 `FLAGO_CONTEXT_MAX_CHARS` 限制。
超过近期窗口或字符预算的旧消息会压缩成当前会话滚动摘要。长期记忆开启时，私聊会话摘要会作为用户可查看、可删除的 `会话摘要` 记忆保存；聊天缓存不会写入长期记忆，系统也不会保存完整聊天原文。
单条长期记忆和每次注入模型上下文的长期记忆总量分别受 `FLAGO_MEMORY_ITEM_MAX_CHARS` 和 `FLAGO_MEMORY_CONTEXT_MAX_CHARS` 限制。
...
会明确提示关闭占用程序或修改 `FLAGO_PORT`。

如果只想双击或从安装器入口打开，可以使用本地启动器脚本：

```powershell
.\.venv\Scripts\python.exe .\scripts\flago_launcher.py
```
...
不要同时运行多个 `flago worker` 或多个启用了长连接的 `flago serve`，否则可能重复消费飞书事件。
...
否则直接查看运行 `uv run flago serve` 的终端输出。
...
- OAuth 回调打不开：确认 `FLAGO_BASE_URL` 是公网 HTTPS 根地址，且飞书后台登记了完全相同的回调 URL。
- OAuth 成功后仍缺权限：确认飞书后台已开通并发布新 scope，然后让用户重新发送 `/授权`。
- 模型无法访问：先确认当前 `FLAGO_DEFAULT_PROVIDER` 是否已注册；OpenAI 兼容 Provider 检查对应 `API_KEY`、`BASE_URL`、`MODEL` 以及 `FLAGO_OPENAI_COMPATIBLE_HTTP_PROXY`；Claude 检查 `ANTHROPIC_*` 配置。
