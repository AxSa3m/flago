...
Flago（FLAGO） 当前已经完成 Gemini 驱动的飞书消息闭环。下一阶段的目标是把模型层从
...
当前代码已新增独立的 `flago.media_workflows` 协议层，不进入普通 `ModelRouter.generate_model`
聊天路径：
...
短期继续使用 `.env`。当前已经实现 `FLAGO_DEFAULT_PROVIDER` 和
`FLAGO_DEFAULT_MODEL`，用于部署级默认 Provider/模型选择；OpenAI-compatible Provider
...
FLAGO_OPENAI_COMPATIBLE_MAX_OUTPUT_TOKENS=
FLAGO_OPENAI_COMPATIBLE_HTTP_PROXY=
...
- test 环境默认使用 `FLAGO_MODEL_TEST_MAX_OUTPUT_TOKENS` 限制输出；dev/prod 默认不硬限制，除非显式设置 Provider 的 max token。
