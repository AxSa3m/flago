...
- `flago.config`：配置加载和环境变量管理。
- `flago.logging`：日志初始化和密钥脱敏。
- `flago.models`：核心 Pydantic 数据模型。
- `flago.storage`：SQLite 存储 token、幂等键、模型偏好和审计日志。
- `flago.feishu`：飞书客户端、OAuth、长连接 worker、OpenAPI 封装和消息路由。
- `flago.gemini`：Gemini 模型提供方，是当前默认真实 Provider。
- `flago.model_providers`：通用模型请求/响应协议、Provider 无关的提示词构建器、注册表和运行时路由。
- `flago.agent`：助手编排层，连接消息、资源、工具和模型；当前同时保留 legacy
  Assistant 与新 AgentOrchestrator。
- `flago.resources`：飞书资源和网页链接解析/读取。
- `flago.writeback`：确认式写回、待确认动作、执行与撤回记录。
- `flago.server`：FastAPI 服务入口。
...
由 `FLAGO_DEFAULT_PROVIDER` 控制，默认模型可通过 `FLAGO_DEFAULT_MODEL` 覆盖；目前已实现
...
- `FLAGO_WRITEBACK_ENABLED=false|true`：关闭时不会保存或执行任何写回。
- `FLAGO_WRITEBACK_AUTO_EXECUTE_ENABLED=false|true`：控制用户是否可以通过 `/写回 自动开启`
  启用个人低风险自动写回；默认关闭。
- `FLAGO_WRITEBACK_CONFIRMATION_MODE=always|low_risk_direct|draft_only`
...
SQLite 默认位置为 `data/flago.sqlite3`，包含：
